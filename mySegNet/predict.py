...
def get_model():
    ...
    model.load_weights('I:\PycharmProjects\Segmantic-segmentation-crackRoad\mySegNet\callbacks\ep009-loss0.083-val_loss0.075.h5')
    return model
...
def predicting(model):
    """ 预测"""
    # 获取测试图片
    test_data_path = r"I:\PycharmProjects\Segmantic-segmentation-crackRoad\dataset\test_img"  # 测试图片的文件夹
    ...
    for test_name in test_data:
        ...
        # (208 * 208, 2) -> (208,208,2)
        predict_picture = predict_picture.reshape((int(HEIGHT - 3), int(WIDHT - 3), CLASS_NUMBERS))
        ...
        # 下面要做的是对合并的图层，分层
        seg_img_array = np.zeros((int(HEIGHT - 3), int(WIDHT - 3), 3))  # (208,208,3) 值全为0
        ...
        # 保存图片
        save_path = r'I:\PycharmProjects\Segmantic-segmentation-crackRoad\dataset'
